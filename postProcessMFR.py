...
class PostProcessPeak:
            ...
            for i in range(len(df.loc[:, self.extractMaxPara])-2):
                if df.loc[i, self.extractMaxPara] < df.loc[i+1, self.extractMaxPara] > df.loc[i+2, self.extractMaxPara]:
                    if df.loc[i+1, self.extractMaxPara] > 1:
                        ...
                        peakTw.append(self.__TwallFunc(self.mTocm*df.loc[i+1, 'Points:0']))
    ...
    def __detectMaximumPeak(self, dfList):
        maxLoc=[]
        maxHRR=[]
    ...
    def __saveMaxCsv(self, maxLoc, maxValue, saveDir, fileName):
        ...
        for i in range(len(maxLoc)-1):
            loc_out = maxLoc[i]
            Twall_out = self.__TwallFunc(loc_out)
    ...
    def __peakGraph_x_t(self, saveDir, *peakdf):
        ...
        #peakdf[0].plot(x='x_cm', y='t_sec', ax=ax, c='r', ls='--', lw=2, label='1st')
        #peakdf[1].plot(x='x_cm', y='t_sec', ax=ax, c='g', ls='--', lw=2, label='2nd')
        #peakdf[2].plot(x='x_cm', y='t_sec', ax=ax, c='b', ls='--', lw=2, label='3rd')
        #peakdf[3].plot(x='x_cm', y='t_sec', ax=ax, c='c', ls='--', lw=2, label='4th')
        ...
        # peakdf[2].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, s=80, c='none', marker='o', alpha=1, lw=2.5, edgecolors='darkmagenta', label='3rd')
        # peakdf[3].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, c='c', alpha=0.7, label='4th')
        ...
        # ax.scatter(peakdf[0].loc[96, 'x_cm'], peakdf[0].loc[96, 't_sec'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='g', label='96')
        ...
        # ax.scatter(peakdf[0].loc[285, 'x_cm'], peakdf[0].loc[285, 't_sec'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='c', label='285')
        # ax.scatter(peakdf[1].loc[285, 'x_cm'], peakdf[1].loc[285, 't_sec'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='c', label='285')
        # ax.scatter(peakdf[0].loc[302, 'x_cm'], peakdf[0].loc[302, 't_sec'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='y', label='302')
        # ax.scatter(peakdf[1].loc[302, 'x_cm'], peakdf[1].loc[302, 't_sec'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='y', label='302')
        
        #peakdf[0].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, s=50, c='HRR_peak', cmap='jet', alpha=0.7)
        #peakdf[1].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, s=50, alpha=0.7)
        #peakdf[2].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, s=50, alpha=0.7)
        #peakdf[3].plot(kind='scatter', x='x_cm', y='t_sec', ax=ax, s=50, alpha=0.7)
        # ax.set_yscale('log')
        #ax.set_xlabel('{}'.format('X (cm)'), FontWeight='bold', FontSize=24)
        ax.set_xlabel('{}'.format('x (cm)'), fontweight='bold', fontsize=24)
        #ax.set_ylabel('{}'.format('Heat Release Rate'), FontWeight='bold', FontSize=24)
        ...
        #ax2 = ax.twinx()
        #ax2.plot(np.linspace(0, 10, 100), self.__TwallFunc(np.linspace(0, 10, 100)), c='k', ls='-', lw=3, alpha=0.2, label='Tw')
    ...
    def __peakGraph_x_HRR(self, saveDir, *peakdf):
        ...
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(12, 8))
        # peakdf[0].plot(x='x_cm', y='HRR_peak', ax=ax, c='r', ls='--', lw=2, label='1st')
        # peakdf[1].plot(x='x_cm', y='HRR_peak', ax=ax, c='g', ls='--', lw=2, label='2nd')
        # peakdf[2].plot(x='x_cm', y='HRR_peak', ax=ax, c='b', ls='--', lw=2, label='3rd')
        # peakdf[3].plot(x='x_cm', y='HRR_peak', ax=ax, c='c', ls='--', lw=2, label='4th')
        
        # peakdf[0].plot(x='x_cm', y='HRR_peak', ax=ax, c='m', alpha=0.8, lw=3, marker='o', ms=10, mew=2, mec='m', label='1st')
        print(peakdf[0].index)
        ...
        # ax.scatter(peakdf[0].loc[96, 'x_cm'], peakdf[0].loc[96, 'HRR_peak'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='g', label='96')
        ...
        # ax.scatter(peakdf[0].loc[285, 'x_cm'], peakdf[0].loc[285, 'HRR_peak'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='c', label='285')
        # ax.scatter(peakdf[1].loc[285, 'x_cm'], peakdf[1].loc[285, 'HRR_peak'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='c', label='285')
        # ax.scatter(peakdf[0].loc[302, 'x_cm'], peakdf[0].loc[302, 'HRR_peak'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='y', label='302')
        # ax.scatter(peakdf[1].loc[302, 'x_cm'], peakdf[1].loc[302, 'HRR_peak'], s=80, c='none', marker='o', alpha=1.0, lw=2.5, edgecolors='y', label='302')
        # peakdf[2].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=80, c='none', marker='v', alpha=1.0, lw=2.5, edgecolors='darkmagenta', label='3rd')
        # peakdf[3].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=40, c='c', alpha=0.4,	lw='2', label='4th')
        
        # peakdf[0].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=100, marker='o', alpha=0.9, lw=2, edgecolors='none', c='HRR_peak', cmap='jet', label='1st')
        # peakdf[1].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=100, marker=',', alpha=0.9, lw=2, edgecolors='none', label='2nd')
        # peakdf[2].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=100, marker='v', alpha=0.9, lw=2, edgecolors='none', label='3rd')
        # peakdf[2].plot(kind='scatter', x='x_cm', y='HRR_peak', ax=ax, s=40, marker='v', alpha=0.8, lw=2, edgecolors='none', label='4th')
        ax.set_yscale('log')
    ...
    def detectPeakValue(self, saveDir):
        # GET MAX HRR
        # maxLoc, maxValue = self.__detectMaximumPeak(self.dfList)
        ...
        # line1, = ax.plot(maxLoc, maxValue, c='r', ls='--', lw=2)
        # plt.savefig("maxHRR_{}_{}.png".format(self.startTime, self.endTime))
        # plt.show()
        ...
        # print(hrr1stPeakList[1])
        # print(hrr1stPeakList[1][0])
        # print(hrr1stPeakList[1])
        #hrrPeakLoc1df, hrrPeakLoc2df, hrrPeakdLoc3f, hrrPeakLoc4df , hrrPeakLoc5df = self.__makeHRRPeakdf(peakdfList)
        #self.__peakHRRgraph(saveDir, ['x_cm', 't_sec'], hrrPeakLoc1df, hrrPeakLoc2df, hrrPeakdLoc3f, hrrPeakLoc4df , hrrPeakLoc5df)
    ...
    def getPropagationSpeed(self, saveDir):
        maxLoc, maxValue = self.__detectMaximumPeak(self.dfList)
        
        ########### substitute 0 when quenching ##############
        maxAll = max(maxValue)
        quenchingPara = 0.01
        ...
        ###################  save results2  ####################
        f2 = open('./{}/result2.csv'.format(saveDir),'w')
        ...
        plt.savefig('./{}/result3propagationSpeed.png'.format(saveDir))
        plt.show()
